...
class SmartMotorController():
    def __init__(self, commandQ, grapQ):
        ...
        self.left_grip_state =["L",False]
        self.right_grip_state =["R",False]
            ...
            while True:
                ...
                try:
                    ...
                    if self.grapQ.qsize()>0:
                        ...
                        if self.grip_state[0] == "R":
                            self.right_grip_state=self.grip_state
                        if self.grip_state[0] == "L":
                            self.left_grip_state=self.grip_state
                    if time.time() - prev_servo_time>=0.05:
                        prev_servo_time = time.time()
                        if self.servo_activate:
                            self.servo_motor.GRIP(self.left_grip_state[0], self.left_grip_state[1])
                            self.servo_motor.GRIP(self.right_grip_state[0], self.right_grip_state[1])
