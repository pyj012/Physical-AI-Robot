...
class GadianRRTPlanner(Node):
    ...
    def _send_goal_pose(self, group_name: str, ee_link: str,
                        ...
                        box_size: float = 0.05,
                        tol_r: float = 0.1, tol_p: float = 0.1, tol_yaw: float = 0.1,
        ...
        if result_wrap.result.error_code.val == 1:
            self.get_logger().info("플래닝 및 실행 성공")
            return True
        else:
            self.get_logger().warn(f"플래닝 실패: error_code={result_wrap.result.error_code.val}")
            return False
    ...
    def plan_and_execute(self, x: float, y: float, z: float) -> bool:
        ...
        ok = self._send_goal_pose(
            ...
            qx, qy, qz, qw,
            box_size=0.01,
            tol_r=0.2, tol_p=0.2, tol_yaw=math.pi,
            plan_only=False
        )
        return ok
