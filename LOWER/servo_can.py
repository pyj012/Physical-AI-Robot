...
class SmartMotorController():
    ...
    def motor_set_pos_spd(self, motor_id, angle, spd, acc):
        ...
        self.bus.send(msg)
    def decode_motor_status(self):
        """
        CAN 메시지로부터 모터의 상태(위치, 속도, 전류 등)를 디코딩합니다.
        """
        ...
        try:
            # 데이터 포맷 '>hhhbb'는 빅 엔디안, 16비트 정수 3개, 8비트 정수 2개를 의미합니다.
            pos_int, spd_int, cur_int, temp, error = struct.unpack(">hhhbb", message.data)
        except struct.error as e:
            print ("Failed to unpack CAN data: {}").format(e)
            return None
        arbitration_id=message.arbitration_id-10496
    ...
    try:
        ...
        while True:
            ...
            if time.time()-prev_time>= 0.1:
                ...
                # if spd> -5000:
                    # spd -=50
            if time.time()-debug_time>= 0.01:
                debug_time = time.time()
                # print(controller.angle_array[0], controller.decode_motor_status())
                # dir = dir * -1

            controller.motor_set_rpm(1,2000)
            time.sleep(0.001)
            # controller.motor_set_rpm(2, spd)
